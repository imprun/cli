...
func TestImprunJSONFieldsAndJQProvideStableMachineOutput(t *testing.T) {
...
func TestImprunTemplateFormatsSelectedOutput(t *testing.T) {
	server := outputTestServer(t)
	defer server.Close()
	t.Setenv("IMPRUN_CONFIG", filepath.Join(t.TempDir(), "config.json"))
...
func TestImprunJSONRejectsUnknownFieldsWithAvailableNames(t *testing.T) {
	server := outputTestServer(t)
	defer server.Close()
	t.Setenv("IMPRUN_CONFIG", filepath.Join(t.TempDir(), "config.json"))
...
func TestImprunTerminalOutputUsesReadableLabels(t *testing.T) {
	server := outputTestServer(t)
	defer server.Close()
	t.Setenv("IMPRUN_CONFIG", filepath.Join(t.TempDir(), "config.json"))
