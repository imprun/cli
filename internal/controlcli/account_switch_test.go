...
func TestImprunAuthSwitchSelectsExistingAccountAfterVerification(t *testing.T) {
...
func TestImprunAuthSwitchDoesNotMutateContextWhenAccountIsMissing(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.json")
	t.Setenv("IMPRUN_CONFIG", configPath)
	config := ConfigFile{
		CurrentProfile: "hosted",
