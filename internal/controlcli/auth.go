...
func (r *runner) auth(args []string) error {
	...
	case "switch":
		if len(args) != 2 {
			return usageError{"usage: imprun auth switch <account>"}
		}
		return r.authSwitch(args[1])
	case "status":
		if len(args) != 1 {
			return usageError{"usage: imprun auth status"}
		}
		return r.authStatus()
	case "logout":
...
func (r *runner) authSwitch(account string) error {
	...
	if !found || strings.TrimSpace(raw) == "" {
		return fmt.Errorf("account %q has no stored credential for this host; run imprun auth login --account %s", account, account)
	}
...
func (r *runner) authLogin(args []string) error {
	...
	if fs.NArg() != 0 {
		return usageError{"usage: imprun auth login [--web | --with-token] [--no-browser] [--account label]"}
	}
	if *withToken && *web {
...
func (r *runner) authLogout(args []string) error {
	...
	if fs.NArg() != 0 {
		return usageError{"usage: imprun auth logout [--local-only]"}
	}
	if r.resolved.ProfileName == "" {
