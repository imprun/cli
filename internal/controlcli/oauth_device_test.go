...
func TestRunImprunHostedDeviceLoginAndRefresh(t *testing.T) {
	...
	t.Setenv("IMPRUN_CONFIG", configPath)
	...
	formMu.Unlock()
	if deviceForm.Get("client_id") != "imprun-cli" ||
	...
	if tokenForm.Get("client_id") != "imprun-cli" ||
	...
	formMu.Unlock()
	if refreshForm.Get("client_id") != "imprun-cli" ||
	...
	formMu.Unlock()
	if revocationForm.Get("client_id") != "imprun-cli" ||
...
func TestRunImprunHostedLoginRevokesCredentialWhenWorkspaceProbeFails(t *testing.T) {
	var server *httptest.Server
	var revocationRequests atomic.Int32
...
func TestRunImprunHostedLogoutPreservesCredentialWhenRemoteRevocationFails(t *testing.T) {
	var revocationRequests atomic.Int32
...
func TestRunImprunHostedLoginRejectsSecretBearingMetadata(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{
			"schema_version": 1,
