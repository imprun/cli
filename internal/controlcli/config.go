...
var (
	...
	imprunProgram = programConfig{
		Name:       "imprun",
		ConfigDir:  "imprun",
		ConfigEnv:  "IMPRUN_CONFIG",
		ContextEnv: "IMPRUN_CONTEXT",
	}
)
...
func configPath() (string, error) {
	return configPathFor(imprunProgram)
}
...
func loadProgramConfig(program programConfig, path string) (ConfigFile, error) {
	if program.Name != imprunProgram.Name || strings.TrimSpace(os.Getenv(program.ConfigEnv)) != "" {
...
func resolveProfile(config ConfigFile, selected string, overrides Profile) (resolvedConfig, error) {
	return resolveProfileFor(imprunProgram, config, selected, overrides)
}
...
func resolveProfileForMode(program programConfig, config ConfigFile, selected string, overrides Profile, validateAPIURL bool) (resolvedConfig, error) {
	...
	if program.Name == imprunProgram.Name {
		contextValues = append(
			contextValues,
			os.Getenv("WF_CONTEXT"),
			os.Getenv("WF_PROFILE"),
			os.Getenv("WINDFORCE_PROFILE"),
		)
	}
	...
	if program.Name == imprunProgram.Name {
		apiURLValues = append(apiURLValues, os.Getenv("IMPRUN_API_URL"), os.Getenv("WF_API_URL"))
		workspaceValues = append(workspaceValues, os.Getenv("IMPRUN_WORKSPACE"), os.Getenv("WF_WORKSPACE"))
		actorValues = append(actorValues, os.Getenv("IMPRUN_ACTOR"), os.Getenv("WF_ACTOR"))
		tokenEnvValues = append(tokenEnvValues, os.Getenv("IMPRUN_TOKEN_ENV"), os.Getenv("WF_TOKEN_ENV"))
	}
	...
	token := firstNonEmpty(os.Getenv("IMPRUN_TOKEN"), os.Getenv("WF_TOKEN"))
	if program.Name != imprunProgram.Name {
		token = ""
	}
	if profile.TokenEnv != "" {
