...
func TestImprunAPIUsesSelectedWorkspaceAndTypedFields(t *testing.T) {
	...
	t.Setenv("IMPRUN_CONFIG", filepath.Join(t.TempDir(), "config.json"))
...
func TestImprunAPIAllowsOnlySelectedHost(t *testing.T) {
...
func TestImprunAPIAbsolutePathRemainsOnContextHost(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
...
func TestImprunAPIRejectsRedirectWithoutForwardingCredentials(t *testing.T) {
	...
	t.Setenv("IMPRUN_CONFIG", filepath.Join(t.TempDir(), "config.json"))
	t.Setenv("IMPRUN_TOKEN", "sensitive-token")
	var stdout, stderr bytes.Buffer
	exit := RunImprun(
		[]string{
			"--api-url", server.URL,
