...
func writeCompletion(writer io.Writer, shell string) error {
	...
	case "bash":
		_, err := fmt.Fprint(writer, imprunBashCompletion)
		return err
	case "zsh":
		_, err := fmt.Fprint(writer, imprunZshCompletion)
		return err
	case "fish":
		_, err := fmt.Fprint(writer, imprunFishCompletion())
		return err
	case "powershell", "pwsh":
		_, err := fmt.Fprint(writer, imprunPowerShellCompletion)
		return err
	default:
		return usageError{"usage: imprun completion bash|zsh|fish|powershell"}
	}
}

const imprunBashCompletion = `_imprun_complete() {
...
complete -F _imprun_complete imprun
`

const imprunZshCompletion = `#compdef imprun
_imprun() {
  local -a commands
  commands=(
...
compdef _imprun imprun
`

func imprunFishCompletion() string {
	var builder strings.Builder
	for _, command := range imprunCompletionCommands[""] {
		fmt.Fprintf(&builder, "complete -c imprun -n '__fish_use_subcommand' -a %s\n", command)
	}
	for _, group := range []string{"auth", "context", "workspace", "source", "app", "release", "action", "run", "job", "provisioning"} {
		for _, command := range imprunCompletionCommands[group] {
			fmt.Fprintf(&builder, "complete -c imprun -n '__fish_seen_subcommand_from %s' -a %s\n", group, command)
		}
	}
	return builder.String()
}

const imprunPowerShellCompletion = `Register-ArgumentCompleter -Native -CommandName imprun -ScriptBlock {
