...
import (
	...
	"github.com/imprun/cli/pkg/controlplane"
)
...
func Run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	return RunImprunWithCredentialStore(args, stdin, stdout, stderr, nil)
}

func RunImprun(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	return RunImprunWithCredentialStore(args, stdin, stdout, stderr, nil)
}

func RunImprunWithCredentialStore(args []string, stdin io.Reader, stdout, stderr io.Writer, store CredentialStore) int {
	return runWithProgram(imprunProgram, args, stdin, stdout, stderr, store)
}
...
func runWithProgramDependencies(
	...
	if program.Name == imprunProgram.Name && remaining[0] == "completion" {
		if len(remaining) != 2 {
			writeError(stderr, usageError{"usage: imprun completion bash|zsh|fish|powershell"})
			return ExitUsage
		}
	...
	if remaining[0] == "profile" || (program.Name == imprunProgram.Name && remaining[0] == "context") {
	...
	if program.Name == imprunProgram.Name && remaining[0] == "auth" {
